			...
			frm.add_custom_button(__('Fetch Orders'), function() {
				if (!frm.doc.location) {
					frappe.msgprint({
						...
						message: __('Choose a Location before fetching orders — the loading plan is built per location.'),
						indicator: 'orange'
					});
					return;
				}
				frappe.confirm(
					'This will replace all current items with fresh data from ' +
						...
						' Sales Orders for ' + frm.doc.delivery_date + '. Continue?',
					function() {
						frappe.call({
							...
							callback: function(r) {
								if (r.message && r.message.status === 'success') {
									...
									frappe.show_alert({
										message: (r.message && r.message.message) || 'Failed to fetch orders',
										indicator: 'red'
									});
								}
							}
