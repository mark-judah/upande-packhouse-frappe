...
def post_transfer(
	*,
	...
	purpose = frappe.db.get_value("Stock Entry Type", entry_type, "purpose") or "Material Transfer"
	...
	cost_center = frappe.db.get_value("Warehouse", source, "custom_cost_center") or default_cost_center(
		company
	)
	if not cost_center:
		...
		for depth, hop in enumerate(
			...
			if hop["terminal"]:
				...
				if here <= QTY_TOLERANCE:
					skipped.append({**hop, "bucket_id": bucket_id, "reason": "bucket already moved"})
					continue
				move = min(qty, here)
			else:
				...
				if here <= QTY_TOLERANCE:
					skipped.append(
						{**hop, "bucket_id": bucket_id, "reason": f"nothing of this order at {hop['from']}"}
					)
					continue
				move = min(qty, here)
...
def post_plans(plans, business_unit):
	...
	for key in sorted(groups, key=lambda k: k[0]):
		...
		entry = post_transfer(
			...
			lines=[{"bucket_id": m["bucket_id"], "qty": m["qty"], "farm": m["farm"]} for m in members],
			farm=head["farm"],
			business_unit=business_unit,
...
def reverse_allocation_movement(sales_order_item, bucket_id=None, item_code=None, business_unit=None):
	...
	for entry in entries:
		...
		for line in doc.items:
			...
			outstanding = sold_qty(line_bucket, line.item_code, sales_order_item, entry.to_warehouse)
...
def mapping_row_for_farm(farm, business_unit):
	...
	for row in frappe.get_doc(MAPPING_DT, name).items:
		if (
			...
			and frappe.db.get_value("Warehouse", row.source_warehouse, "custom_farm") == farm
		):
			return row
	return None
	...
	*,
	...
	if box_label:
		if box_leg_posted(box_label, item_code, source, target):
			return {"entry": None, "qty": 0, "reason": f"box {box_label} already moved {source} -> {target}"}
