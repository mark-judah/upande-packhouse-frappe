	...
	for hop in resolve_route(source, business_unit, graded=graded):
		...
		if hop["terminal"]:
			...
			if move <= QTY_TOLERANCE:
				results.append({**hop, "entry": None, "qty": 0, "skipped": "already sold"})
				continue
		else:
			here = bucket_balance(bucket_id, item_code, hop["from"])
			if here <= QTY_TOLERANCE:
				results.append({**hop, "entry": None, "qty": 0, "skipped": "bucket already moved"})
				continue
			move = min(qty, here)
...
def backfill_shelf_arrivals(business_unit="Roses", dry_run=True, limit=None):
	...
	for r in rows:
		...
		try:
			route = [h for h in resolve_route(r.warehouse, business_unit) if not h["terminal"]]
			if not route:
