...
def create_mixed_box_pick_list_for_allocated_items(sales_order_doc, allocations, submit=True, location=None):
	...
	frappe.log_error(title="Mixed Box OPL - Farm", message=f"Location: {location} → Shelf Farm: {shelf_farm}")
		...
		mixed_box_allocations[mix_group].append(
			{
				"alloc": {
					...
					"available_exact_stems": alloc_list[0].get("available_exact_stems", 0)
					if alloc_list
					else 0,
				},
				"so_item": so_item,
	...
	for mix_group, box_items in mixed_box_allocations.items():
		...
		all_items_in_mix = frappe.get_all(
			"Sales Order Item",
			filters={"parent": sales_order_doc.name, "custom_mix_group": mix_group, "custom_mixed_box": 1},
			fields=[
				"name",
				"item_code",
				"item_name",
		...
		existing_opl = frappe.get_all(
			"Order Pick List",
			filters={"sales_order": sales_order_doc.name, "mix_group": mix_group, "docstatus": ["!=", 2]},
		...
		if existing_opl:
			...
			for item in box_items:
				...
				existing_rows = [
					loc for loc in order_pick_list.table_ytkc if loc.sales_order_item == so_item_name
				]
			...
			group_items = [
				it
				for it in sales_order_doc.items
...
def _generate_box_locations_with_splitting(
	alloc,
	so_item,
	sales_order_name,
	sales_order_item_name,
...
def _lookup_shelf(actual_farm, item_code, bucket_id, fallback_farm):
	...
	matching_shelves = frappe.db.sql(
        ...
        LIMIT 3
    """,
		[actual_farm, item_code, bucket_id],
		as_dict=True,
	)
	...
	if not matching_shelves and actual_farm != fallback_farm:
		matching_shelves = frappe.db.sql(
            ...
            LIMIT 3
        """,
			[fallback_farm, item_code, bucket_id],
			as_dict=True,
		)
