...
def create_mixed_bunch_pick_list_for_allocated_items(
	...
	for bunch_group, box_items in bunch_allocations.items():
		...
		existing_opl = frappe.get_all(
			"Order Pick List",
			filters={"sales_order": sales_order_doc.name, "bunch_group": bunch_group, "docstatus": ["!=", 2]},
...
def _generate_bunch_flat_locations(
	alloc,
	so_item,
	sales_order_name,
	sales_order_item_name,
	...
	for alloc_entry in allocations_list:
		...
		locations.append(
			_bunch_row(
				...
				bucket_id,
				stems,
				alloc_entry.get("warehouse"),
				alloc_entry.get("downgrade_reason", ""),
				alloc_entry.get("available_exact_stems", 0),
				awaiting_transfer,
				shelf_str,
				actual_farm,
				conversion_factor,
...
def _generate_bunch_locations_with_splitting(
	alloc,
	so_item,
	sales_order_name,
	sales_order_item_name,
		...
		for contrib in contributions:
			...
			locations.append(
				_bunch_row(
					...
					contrib["stems"],
					contrib["warehouse"],
					contrib.get("downgrade_reason", ""),
					contrib.get("available_exact_stems", 0),
					awaiting_transfer,
					shelf_str,
					actual_farm,
					conversion_factor,
