...
def _get_shelf_farm_for_location(location):
	...
	enabled_sales_farms = [row.farm for row in (ps.shelf_locations or []) if row.enabled and row.sales_shelf]
...
def create_straight_box_pick_list_for_allocated_items(
	sales_order_doc, allocations, submit=True, location=None
):
	"""
	...
	frappe.log_error(
		title="Straight OPL Creation - Farm", message=f"Location: {location} → Shelf Farm: {shelf_farm}"
	)
	...
	try:
		for sales_order_item_name, allocs in allocations_by_so_item.items():
			so_item = next((i for i in sales_order_doc.items if i.name == sales_order_item_name), None)
			if not so_item:
				frappe.throw(f"Sales Order Item {sales_order_item_name} not found in {sales_order_doc.name}")
			...
			try:
				...
				if submitted:
					...
					frappe.msgprint(
						f"Pick List {order_pick_list.name} created and submitted.", indicator="green"
					)
				else:
					reason = "awaiting transfer" if any_awaiting else "partial allocation"
					frappe.msgprint(
						f"Pick List {order_pick_list.name} created as draft ({reason}).", indicator="blue"
					)

				order_pick_list_names.append(order_pick_list.name)

			except Exception as e:
				frappe.log_error(f"Failed to create OPL for SO Item {sales_order_item_name}: {e}")
				raise

	except Exception as e:
		frappe.log_error(f"Error creating straight OPLs for {sales_order_doc.name}: {e}")
		raise

	if order_pick_list_names:
		frappe.msgprint(
			f"Created {len(order_pick_list_names)} Pick List(s) for {sales_order_doc.name}", indicator="green"
		)
		try:
			frappe.db.set_value(
				"Sales Order",
