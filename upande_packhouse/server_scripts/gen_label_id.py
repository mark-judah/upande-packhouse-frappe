...
def generate_batch_table_labels(docname):
	...
	if frappe.db.get_value("Label Print", docname, "labels_generated"):
		frappe.throw(
			_(
				"Labels have already been generated for {0}. If the PDF never showed up, use the "
				'"Generate Attachment" button instead of saving again. For a new batch, create a '
				"new Label Print document."
			).format(frappe.bold(docname)),
...
def _download_links_text(file_urls):
	...
	return " &middot; ".join(
		f"<a href='{url}' target='_blank'>Part {i}</a>" for i, url in enumerate(file_urls, start=1)
	)
...
def salvage_batch_table_attachment_job(docname):
	...
	try:
		...
		notification_doc.subject = f"Attachment generation failed: {docname}"
		notification_doc.email_content = (
			f"An error occurred while regenerating the attachment for {docname}. Please contact admin."
		)
		notification_doc.document_type = "Label Print"
		notification_doc.document_name = docname
		notification_doc.insert(ignore_permissions=True)
...
def run_label_generation_job(docname):
	...
	try:
		...
		if total_count == 0:
			...
			_set_progress(
				docname, 0, _("Generating Labels"), _("Nothing to do -- every row's quantity was 0.")
			)
			return
		...
		row_farms = sorted(
			{(row.farm or parent_doc.farm_name) for row in valid_rows if (row.farm or parent_doc.farm_name)}
		)
		...
		if missing_farm_codes:
			...
			message = _(
				...
				", ".join(frappe.bold(f) for f in missing_farm_codes),
			...
			_set_progress(
				docname, 0, _("Generating Labels"), _("Failed -- Farm Code missing, see notification.")
			)
			return
		...
		_set_progress(
			...
			100,
			_("Generating Labels"),
			_("Done -- {0:,} labels").format(total_count)
			if pdf_file_urls
		...
		notification_doc.email_content = (
			f"An error occurred while generating labels. Please contact admin. Error: {str(e)[:100]}"
		)
...
def generate_batch_labels_pdf_pymupdf(label_data_list, parent_doc_name):
	...
	try:
		...
		frappe.log_error(
			f"PyMuPDF: Generated PDF with {len(label_data_list)} labels", "PDF Generation Success"
		)
		return pdf_base64
...
def attach_batch_labels_pdf(label_data_list, docname, doctype, filename=None, on_progress=None):
	...
	try:
		for part, group in enumerate(groups, start=1):
			part_name = (
				f"{base_name}.pdf" if total_parts == 1 else f"{base_name}_part{part}_of_{total_parts}.pdf"
			)
...
def get_batch_labels_pdf(docname):
	...
	try:
		...
		missing_farm_codes = sorted(
			{label.get("farm") for label in labels if label.get("farm") and not label.get("farm_code")}
		)
		if missing_farm_codes:
			frappe.throw(
				_(
