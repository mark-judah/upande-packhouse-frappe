...
HARVESTER = "500201"
GH = {
	"Odilia": "Kapkolia GH 18 - KR",
	"Alicia": "Kapkolia GH 18 - KR",
	"Mirabel": "Kapkolia GH 18 - KR",
	"Marisa": "Kapkolia GH 18 - KR",
...
def _receive_and_shelve(bucket_id, shelf_id):
	recv = _call(
		"upande_quality.mobile.api.createReceivingStockEntry",
		{
			"bucket_id": bucket_id,
			"custom_receiving_batch_id": None,
			"confirm_receive": True,
		},
	)
	shelf = _call(
		"upande_quality.mobile.api.createShelvingEntry",
		{
			"farm": FARM,
			"shelf_id": shelf_id,
			"bucket_id": bucket_id,
	...
	for variety, cut_stage, length, n_buckets, bunches_per_bucket in SPRAY_PLAN:
		...
		for b in range(1, n_buckets + 1):
			...
			for j in range(bunches_per_bucket):
				...
				idx += 1
				grade_resp = _call(
					"upande_agriculture.mobile.api.createGradingStockEntry",
					{
						"farm": FARM,
				...
				he = (
					frappe.db.get_value(
						"Stock Entry",
						{"custom_grading_entry": grading_entry},
						"name",
					)
					if grading_entry
					else None
			...
			for he in harvest_entries_this_bucket:
				amend = _call(
					"upande_agriculture.mobile.api.amendHarvestEntry",
					{
						"stock_entry_name": he,
						"cut_stage": cut_stage,
					},
