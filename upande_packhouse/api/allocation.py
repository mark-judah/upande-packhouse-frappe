...
def fetchSalesAllocationPlanningData():
	...
	confirmed_stems_query = """
        ...
        WHERE cs.parenttype = 'Sales Order'
            ...
            AND so2.delivery_date = %(delivery_date)s
    """
	confirmed_stems_data = frappe.db.sql(
		confirmed_stems_query, {"delivery_date": delivery_date}, as_dict=True
	)
	...
	for cs in confirmed_stems_data:
		...
		confirmed_stems_map[key].append({"farm": cs["farm"], "stems": cs["stems"]})
	...
	def update_processing_location():
		...
		try:
			...
			if action == "confirm":
				...
				other_confirmed = sum(
					(entry.stems or 0) for entry in line_entries if entry.farm != processing_location
				)
				...
				if stems_val == 0:
					...
					sales_order_doc.append(
						"custom_confirmed_stems_table",
						{"farm": processing_location, "stems": stems_val, "sales_order_item": item_row_name},
					)
