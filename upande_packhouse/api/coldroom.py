	...
	try:
		...
		try:
			settings_rows = frappe.db.sql(
				"SELECT amber_time, discard_age FROM `tabProduction Settings` LIMIT 1", as_dict=True
			)
			if settings_rows:
...
def getColdroomBuckets():
	...
	try:
		...
		req = frappe.db.sql(
            ...
            FROM `tabOrder Pick List` opl
            INNER JOIN `tabPick List Item` pli ON pli.parent = opl.name
            WHERE opl.docstatus = 1 AND opl.date_created = CURDATE() AND IFNULL(pli.issued, 0) = 0"""
			+ fw_opl
			+ """
