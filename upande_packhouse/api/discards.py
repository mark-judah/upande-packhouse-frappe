...
def getDiscardData():
	...
	by_variety = frappe.db.sql(
		...
		+ BASE
		+ " GROUP BY drb.variety ORDER BY stems DESC LIMIT 8",
		params,
		as_dict=True,
	)
	...
	by_farm = frappe.db.sql(
		...
		+ BASE
		+ " GROUP BY drb.farm ORDER BY stems DESC LIMIT 8",
		params,
		as_dict=True,
	)
	...
	if need_item and need_len and val_currency:
		...
		sql = (
			"SELECT item_code, custom_length, price_list_rate, valid_from, valid_upto "
			"FROM `tabItem Price` WHERE selling = 1 AND price_list = %(vpl)s "
			"AND item_code IN "
			+ inlist("it_", list(need_item.keys()))
			+ " AND custom_length IN "
			+ inlist("ln_", list(need_len.keys()))
		)
		cand = {}
