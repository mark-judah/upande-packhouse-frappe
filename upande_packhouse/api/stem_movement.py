		...
		if not box_name:
			...
			if not box:
				...
				out["box"] = {
					...
					"po": box.get("customer_purchase_order"),
					"pack_rate": box.get("pack_rate"),
				}
				for it in box.get("box_item") or []:
					out["box_items"].append(
						{"variety": it.get("variety"), "length": it.get("length"), "qty": it.get("qty")}
					)
				...
				if opl_name:
					ov = frappe.get_all(
						"Order Pick List",
						filters={"name": opl_name},
						fields=["team", "consignee", "farm"],
						limit=1,
					)
					if ov:
				...
				if box.get("farm"):
					...
					for h in harvests:
						...
						if vk not in g["varieties"]:
							g["varieties"][vk] = {
								"variety": h.get("item_code", ""),
								"length": h.get("stem_length", ""),
								"qty": 0,
							}
					...
					for gn in sorted(gh_map.keys()):
						...
						res.append(
							{
								"name": gd["name"],
								"total_stems": gd["total_stems"],
								"bucket_count": len(gd["buckets"]),
								"varieties": vlist,
							}
						)
					out["greenhouses"] = res
				...
				if opl_name:
					locs = frappe.get_all(
						...
						fields=["bucket", "stock_qty", "qty", "custom_box_id"],
						limit_page_length=0,
					)
					for loc in locs:
						bid = loc.get("bucket")
						...
						if boxid:
							packed_map[bid]["packed"] = packed_map[bid]["packed"] + (
								loc.get("stock_qty") or 0
							)
				...
				for bid in bucket_ids:
					...
					if pm:
						...
						if pm.get("packed"):
							stage["packed"] = {
								"stems": int(pm["packed"]),
								"bunches": int(pm.get("bunches") or 0),
								"boxes": len(pm["boxes"]),
							}
					...
					try:
						...
						if hv:
							...
							stage["harvested"] = {
								...
								"greenhouse": r.get("custom_greenhouse", ""),
								"harvester": r.get("custom_harvester", ""),
							}
					except Exception:
						pass
					...
					try:
						...
						if rc:
							...
							action = (
								"Quarantined"
								if "Quarantined" in tp
								else ("Accepted" if "Accepted" in tp else "Received")
							)
							stage["received"] = {
								"date": str(r.get("posting_date", "")),
								"action": action,
								"warehouse": r.get("to_warehouse", ""),
							}
					except Exception:
						pass
					...
					try:
						gr = frappe.get_all(
							...
							filters={
								"custom_bucket_id": bid,
								"stock_entry_type": ["in", ["Grading", "Grading Forecast"]],
								"docstatus": 1,
							},
							fields=["posting_date", "to_warehouse"],
							order_by="posting_date desc",
							limit=1,
						)
						if not gr:
					...
					try:
						sl = frappe.get_all(
							...
							fields=["shelf", "stem_qty", "shelved_on"],
							order_by="shelved_on asc",
							limit=1,
						)
						if sl:
							r = sl[0]
						...
						if dsc:
							...
							try:
								dd = frappe.get_all(
									...
									fields=["custom_rejection_reason"],
									limit=1,
								)
								if dd:
	...
	try:
		...
		if varieties:
			for r in frappe.get_all(
				"Item",
				filters={"name": ["in", list(varieties)]},
		...
		summary = {
			...
			"not_shelved_stems": sum(
				(b["received_stems"] or b["harvested_stems"]) for b in result if not b["shelved"]
			),
		}
	...
	try:
		...
		if group_param:
			group_items = set(
				r["name"]
				for r in frappe.get_all(
					"Item", filters={"item_group": group_param}, fields=["name"], limit_page_length=0
				)
			)
		...
		def _buf_where(col_farm, col_var, col_len):
			...
			if farm_param and col_farm:
				cl.append(col_farm + " LIKE %(bf)s")
				p["bf"] = farm_param + "%"
			if variety_param and col_var:
				cl.append(col_var + " = %(bv)s")
				p["bv"] = variety_param
			if length_param and col_len:
				cl.append(col_len + " = %(bl)s")
				p["bl"] = length_param
		...
		in_transit = {
			"level": int(r.lvl or 0),
			"buckets": int(r.buckets or 0),
			"age_hours": _age_hours(r.oldest),
		}
		...
		def _box_buf(cond):
			...
			return {
				"level": int(row.lvl or 0),
				"buckets": int(row.n or 0),
				"age_hours": _age_hours(row.oldest),
			}
		...
		cohort_params = dict(params)
		cohort_params.update(cp)
		crow = frappe.db.sql(
			"""
            SELECT
		...
		if 1 <= ndays <= 92:
			...
			_buf_gates = [
				("in_transit", "harvested", "received"),
				("unshelved", "received", "shelved"),
				("on_shelf", "shelved", "issued"),
				("pack_hall", "issued", "packed"),
				("awaiting_stage", "packed", "staged"),
				("staged", "staged", "loaded"),
			...
			if compare_from and compare_to:
				...
				if (cd1 - cd0).days + 1 == ndays:
					...
					trends["compare_gates"] = {
						st: [round(cpd.get(dt, {}).get(st, 0)) for dt in cdates] for st in STAGES
					}
...
def getPackhouseFlowBoard():
	...
	try:
		...
		def _flows(f, t):
			...
			q(
				"SELECT COALESCE(SUM(sed.qty),0) s FROM `tabStock Entry` se JOIN `tabStock Entry Detail` sed ON sed.parent=se.name WHERE se.docstatus=1 AND se.stock_entry_type='Harvesting' AND se.posting_date BETWEEN %(f)s AND %(t)s"
				+ _frag("se.farm", "sed.item_code", "se.custom_greenhouse"),
				"harvested",
			)
			q(
				"SELECT COALESCE(SUM(sed.qty),0) s FROM `tabStock Entry` se JOIN `tabStock Entry Detail` sed ON sed.parent=se.name WHERE se.docstatus=1 AND se.stock_entry_type IN ('Receiving','Late Receipt') AND se.posting_date BETWEEN %(f)s AND %(t)s"
				+ _frag("se.farm", "sed.item_code", "se.custom_greenhouse"),
				"received",
			)
			q(
				"SELECT COALESCE(SUM(CASE WHEN EXISTS(SELECT 1 FROM `tabShelving Log` sl WHERE sl.bucket_id=se.custom_bucket_id AND sl.shelved_on>=se.posting_date AND sl.shelved_on<=%(endt)s) THEN sed.qty ELSE 0 END),0) s FROM `tabStock Entry` se JOIN `tabStock Entry Detail` sed ON sed.parent=se.name WHERE se.docstatus=1 AND se.stock_entry_type IN ('Receiving','Late Receipt') AND se.posting_date BETWEEN %(f)s AND %(t)s"
				+ _frag("se.farm", "sed.item_code", "se.custom_greenhouse"),
				"shelved",
			)
			q(
				"SELECT COALESCE(SUM(pli.stock_qty),0) s FROM `tabPick List Item` pli JOIN `tabOrder Pick List` opl ON opl.name=pli.parent AND pli.parenttype='Order Pick List' WHERE pli.issued=1 AND opl.date_created BETWEEN %(f)s AND %(t)s"
				+ _frag("opl.farm", "pli.item_code", None),
				"issued",
			)
			q(
				"SELECT COALESCE(SUM(pli.stock_qty),0) s FROM `tabFarm Pack List` fpl JOIN `tabFarm Packlist Item` pli ON pli.parent=fpl.name AND pli.parenttype='Farm Pack List' AND pli.parentfield='pack_list_item' WHERE fpl.docstatus!=2 AND DATE(fpl.creation) BETWEEN %(f)s AND %(t)s"
				+ _frag("pli.source_warehouse", "pli.item_code", None),
				"packed",
			)
			q(
				"SELECT COALESCE(SUM(bi.qty),0) s FROM `tabBox Label` bl JOIN `tabBox Label Item` bi ON bi.parent=bl.name JOIN `tabSales Order` so ON so.name=bl.customer_purchase_order WHERE bl.staged=1 AND so.docstatus=1 AND DATE(bl.date) BETWEEN %(f)s AND %(t)s AND so.status NOT IN ('Cancelled','Closed')"
				+ _frag("bl.farm", "bi.variety", None),
				"staged",
			)
			q(
				"SELECT COALESCE(SUM(pli.stock_qty),0) s FROM `tabPick List Item` pli JOIN `tabOrder Pick List` opl ON opl.name=pli.parent AND pli.parenttype='Order Pick List' WHERE pli.loaded_in_trolley=1 AND opl.date_created BETWEEN %(f)s AND %(t)s"
				+ _frag("opl.farm", "pli.item_code", None),
				"loaded",
			)
			q(
				"SELECT COALESCE(SUM(dni.stock_qty),0) s FROM `tabDelivery Note` dn JOIN `tabDelivery Note Item` dni ON dni.parent=dn.name WHERE dn.docstatus=1 AND dn.posting_date BETWEEN %(f)s AND %(t)s"
				+ _frag("dni.farm", "dni.item_code", None),
				"dispatched",
			)
			return tt
		...
		in_transit = one(
			"""SELECT COALESCE(SUM(sed.qty),0) lvl, MIN(se.creation) oldest
		...
		on_shelf = one(
			"""SELECT COALESCE(SUM(si.stem_qty),0) lvl, MIN(COALESCE(si.receiving_date,si.date_added)) oldest
			...
			+ _frag("s.farm", "si.variety", None)
		)
		pack_hall = one(
			"""SELECT COALESCE(SUM(pli.stock_qty),0) lvl, MIN(opl.creation) oldest
		...
		def box_buf(cond):
			return one(
				"SELECT COALESCE(SUM(bi.qty),0) lvl, MIN(bl.creation) oldest FROM `tabBox Label` bl JOIN `tabBox Label Item` bi ON bi.parent=bl.name WHERE "
				+ cond
				+ " AND bl.date>=DATE_SUB(CURDATE(),INTERVAL 7 DAY)"
				+ _frag("bl.farm", "bi.variety", None)
			)
		...
		sh_shelved_in = slq(
			"SELECT COALESCE(SUM(stem_qty),0) s FROM `tabShelving Log` WHERE DATE(shelved_on)=%(d)s",
			{"d": today},
		)
		sh_issued_out = slq(
			"SELECT COALESCE(SUM(stem_qty),0) s FROM `tabShelving Log` WHERE DATE(removed_on)=%(d)s AND (reason LIKE '%%Issue%%' OR reason LIKE '%%Pick%%' OR reason LIKE '%%Sales%%')",
			{"d": today},
		)
		sh_discard_out = slq(
			"SELECT COALESCE(SUM(stem_qty),0) s FROM `tabShelving Log` WHERE DATE(removed_on)=%(d)s AND NOT (reason LIKE '%%Issue%%' OR reason LIKE '%%Pick%%' OR reason LIKE '%%Sales%%' OR reason LIKE '%%Shelved%%')",
			{"d": today},
		)
		sh_opening = slq(
			"SELECT COALESCE(SUM(stem_qty),0) s FROM `tabShelving Log` WHERE shelved_on < %(m)s AND (removed_on IS NULL OR removed_on >= %(m)s)",
			{"m": morning},
		)
		sh_now = int(on_shelf.lvl or 0)
		sh_open_oldest = one(
			"SELECT MIN(shelved_on) oldest FROM `tabShelving Log` WHERE shelved_on < %(m)s AND (removed_on IS NULL OR removed_on >= %(m)s)",
			{"m": morning},
		).oldest
		...
		events = frappe.db.sql(
			"""SELECT HOUR(shelved_on) h, SUM(stem_qty) q, 'in' k FROM `tabShelving Log` WHERE DATE(shelved_on)=%(d)s GROUP BY HOUR(shelved_on)
            UNION ALL SELECT HOUR(removed_on) h, SUM(stem_qty) q, 'out' k FROM `tabShelving Log` WHERE DATE(removed_on)=%(d)s GROUP BY HOUR(removed_on)""",
			{"d": today},
			as_dict=True,
		)
		delta = {}
		...
		bands_raw = frappe.db.sql(
            ...
            FROM `tabShelf` s JOIN `tabShelf Item` si ON s.name=si.parent WHERE si.stem_qty>0"""
			+ _frag("s.farm", "si.variety", None)
			+ " GROUP BY band",
			flt,
			as_dict=True,
		...
		discard_reasons = [
			{"reason": r.reason, "qty": int(r.q or 0)}
			for r in frappe.db.sql(
				"SELECT reason, SUM(stem_qty) q FROM `tabShelving Log` WHERE DATE(removed_on)=%(d)s AND NOT (reason LIKE '%%Issue%%' OR reason LIKE '%%Shelved%%' OR reason LIKE '%%Pick%%' OR reason LIKE '%%Sales%%') GROUP BY reason ORDER BY q DESC",
				{"d": today},
				as_dict=True,
			)
		]
		...
		buffers = [
			...
			build_buf(
				"unshelved",
				"In the cold store",
				"Received, not yet shelved",
				unshelved,
				"received",
				[("shelved", totals["shelved"])],
			),
			build_buf(
				"on_shelf",
				"On the shelf",
				"Shelved, not yet issued",
				on_shelf,
				"shelved",
				[("issued", totals["issued"]), ("discarded", sh_discard_out)],
			),
			build_buf(
				"pack_hall",
				"In the packhouse",
				"Issued, not yet packed",
				pack_hall,
				"issued",
			...
			build_buf(
				"awaiting_stage",
				"Packed, waiting to stage",
				"Packed, not yet staged",
				awaiting,
				"packed",
		...
		for b in buffers:
			if b["key"] == "on_shelf":
				...
				b["outflow"] = [
					{"label": "issued", "qty": sh_issued_out},
					{"label": "discarded", "qty": sh_discard_out},
				]
		...
		shelf = {
			"opening": sh_opening,
			"shelved_in": sh_shelved_in,
			"issued_out": sh_issued_out,
			"discarded_out": sh_discard_out,
			"now": sh_now,
			"reconciled": sh_reconciled,
			"variance": sh_now - sh_reconciled,
			"opening_oldest_h": _age(sh_open_oldest),
			"now_oldest_h": _age(on_shelf.oldest),
			"buckets_in": slq(
				"SELECT COUNT(DISTINCT bucket_id) s FROM `tabShelving Log` WHERE DATE(shelved_on)=%(d)s",
				{"d": today},
			),
			"age_bands": age_bands,
			"threshold_h": 48,
		...
		for i in (2, 1, 0):
			di = str(d0 - timedelta(days=i))
			days.append(
				{"label": frappe.utils.getdate(di).strftime("%a %d"), "today": (i == 0), "t": day_flows[di]}
			)
		...
		crow = frappe.db.sql(
            ...
            FROM (SELECT se.custom_bucket_id bucket, SUM(sed.qty) stems, MIN(se.posting_date) hdate
              ...
              WHERE se.docstatus=1 AND se.stock_entry_type='Harvesting' AND se.posting_date=%(d)s
                AND se.custom_bucket_id IS NOT NULL AND se.custom_bucket_id<>'' """
			+ _frag("se.farm", "sed.item_code", "se.custom_greenhouse")
			+ """ GROUP BY se.custom_bucket_id) h
        """,
			{**flt, "d": today},
			as_dict=True,
		)[0]
		reached = {
			"harvested": int(crow.harvested or 0),
			"received": int(crow.received or 0),
			"shelved": int(crow.shelved or 0),
			"issued": int(crow.issued or 0),
			"packed": int(crow.packed or 0),
			"staged": 0,
			"loaded": 0,
			"dispatched": 0,
		}
		...
		farm_opts = _distinct(
			"farm",
			"""SELECT DISTINCT farm v FROM `tabStock Entry`
		...
		var_opts = _distinct(
			"variety",
			"""SELECT DISTINCT sed.item_code v
            FROM `tabStock Entry` se JOIN `tabStock Entry Detail` sed ON sed.parent = se.name
		...
		loc_opts = _distinct(
			"location",
			"""SELECT DISTINCT custom_greenhouse v FROM `tabStock Entry`
            WHERE stock_entry_type IN ('Harvesting','Receiving','Late Receipt')
              AND posting_date >= DATE_SUB(CURDATE(), INTERVAL 60 DAY)
