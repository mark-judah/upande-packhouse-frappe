...
def getDashboardData():
	...
	try:
		...
		if len(opl_names) > 0:
			...
			item_group_map = {}
			if len(pli_codes) > 0:
				ig_rows = frappe.get_all(
					"Item", filters=[["name", "in", pli_codes]], fields=["name", "item_group"]
				)
				ig_i = 0
		...
		while i < len(all_opls):
			...
			row = {
				...
				"rose_type": (
					"Mixed"
					if (spray_stems_by_opl.get(oid, 0) > 0 and std_stems_by_opl.get(oid, 0) > 0)
					else (
						"Spray Roses"
						if spray_stems_by_opl.get(oid, 0) > 0
						else ("Standard Roses" if std_stems_by_opl.get(oid, 0) > 0 else "")
					)
				),
			}
		...
		global_issuing_pct = (
			round(total_all_issued_stems / total_all_pick_stems * 100) if total_all_pick_stems > 0 else 0
		)

		# Packing efficiency: of total planned stems, what % got packed
		global_packing_pct = (
			round(total_all_packed_stems / total_all_planned_stems * 100)
			if total_all_planned_stems > 0
			else 0
		)
