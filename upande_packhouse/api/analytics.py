		...
		greenhouse_chain.append(
			{
				"greenhouse": gh,
				"gh_kr": gh
				+ " - KR",  # Quality Reporting.ghouse has no farm suffix; Scouting Entry.greenhouse does
				"ph_checked": checked,
				"ph_rejected": rejected,
...
def getConsumablesStock():
	...
	if customers_needed:
        ...
        """,
			...
			latest_spec_by_customer.setdefault(
				r.customer, r.name
			)  # first hit per customer = latest (DESC order)
	...
	for item_code, d in demand_by_item.items():
		...
		demand_vs_stock.append(
			{
				"item_code": item_code,
				"item_name": d["item_name"] or item_code,
				"needed_for_open_orders": needed,
	...
	frappe.response["message"] = {
		...
		"top_items": top_items,
		"trend": [
			{"date": str(r.date), "issued": round(flt(r.issued)), "received": round(flt(r.received))}
			for r in trend
		],
		"material_requests": [
			{
...
def getSalesOverview():
	...
	top_customer_share_pct = (
		round((top_customer_rows[0][1] / total_orders) * 100, 1) if total_orders and top_customer_rows else 0
	)
	concentration_rows = [
		{
			"customer": c,
	...
	frappe.response["message"] = {
		...
		"currency_mix": [
			{"currency": k, "revenue": round(v, 2)}
			for k, v in sorted(currency_totals.items(), key=lambda kv: kv[1], reverse=True)
		],
...
def getConversionData():
		...
		per_order.append(
			{
				...
				"packed": packed,
				"conversion_pct": pct,
				"value_outstanding": value_outstanding,
				"currency": o.currency,
			}
		)
	...
	frappe.response["message"] = {
		...
		"to_date": to_date,
		"totals": {
			"ordered": round(total_ordered),
			"allocated": round(total_allocated),
...
def getDowngradeAnalysis():
	...
	frappe.response["message"] = {
		...
		"downgrade_rate_pct": round((total_downgraded_stems / total_picked_stems) * 100, 1)
		if total_picked_stems
		else 0,
...
def getOrdersAtRisk():
    ...
    """,
		{"from_floor": from_floor, "to_date": to_date},
		as_dict=True,
	)
		...
		results.append(
			{
				...
				"available_now": round(available_here),
...
def getAgeToAllocation():
	...
	variety_rows = [
		{"variety": v, "avg_age": round(sum(a) / len(a), 1), "count": len(a)}
		for v, a in by_variety.items()
		if len(a) >= 2
	]
