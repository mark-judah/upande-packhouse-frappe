...
#
# A bunch is graded once and its QR label carries that length for life. Sales
# allocation can then allocate the bunch's BUCKET to an order asking for a
# SHORTER length -- the stems are cut down at packing -- so the label and the
# order legitimately disagree and the scan must not be refused for it.
#
# The reverse never holds: a 50cm bunch cannot fill a 60cm line, no matter what
# the paperwork says. So the rule is "the bunch's own bucket is on this pick
# list AND the order wants that length or shorter", and an upgrade stays a
# mismatch.
# ──────────────────────────────────────────────────────────────────────────────

# Fields every bunch lookup needs to answer "may this be packed here?".
BUNCH_GRADING_FIELDS = [
	"name",
	"custom_bunch_id",
...
def _length_cm(value):
	"""Parse a stem length ('60cm', '60', ' 60 CM ') to an int. 0 when unusable."""
	if not value:
		return 0
	try:
		return int(str(value).lower().replace("cm", "").strip())
	except (ValueError, TypeError):
		return 0


def _bunch_packing_target(grading_entry, opl_name=None):
	"""Length a scanned bunch may be packed at, or None to leave it alone.

	`grading_entry` is a Stock Entry row carrying at least custom_stem_length and
	custom_bucket_id -- the length the bunch was graded (and labelled) at, and
	the bucket it came from. Nothing rewrites those, so custom_stem_length is
	always the ORIGINAL length here.

	Looks up the pick rows holding that same bucket -- restricted to the pick
	list being packed when the caller knows it -- reads what each row's Sales
	Order Item actually asks for, and accepts a row only when the order wants
	the bunch's length or less. Where several qualify the shallowest downgrade
	wins: packing a 70cm bunch into a 60cm line while a 65cm line is also open
	throws away 5cm for nothing.

	Returns (target_length, detail). target_length is None when no row on this
	pick list can take the bunch; detail["upgrade_only"] then says whether that
	was because every matching row wanted a LONGER stem than the bunch has,
	which is the one case that should still be reported as a mismatch.
	"""
	graded = grading_entry.get("custom_stem_length") if grading_entry else None
	bucket = grading_entry.get("custom_bucket_id") if grading_entry else None

	detail = {
		"bucket": bucket,
		"graded_stem_length": graded,
		"candidates": [],
		"upgrade_only": False,
	}

	graded_cm = _length_cm(graded)
	if not bucket or not graded_cm:
		return None, detail

	conditions = ["pli.bucket = %(bucket)s", "pli.docstatus = 1", "pli.parenttype = 'Order Pick List'"]
	params = {"bucket": bucket}
	if opl_name:
		conditions.append("pli.parent = %(opl)s")
		params["opl"] = opl_name

	rows = frappe.db.sql(
		"""
		SELECT pli.parent AS opl, pli.stem_length AS pick_length, soi.custom_length AS order_length
		FROM `tabPick List Item` pli
		LEFT JOIN `tabSales Order Item` soi
		       ON soi.name = COALESCE(pli.custom_sale_order_item, pli.sales_order_item)
		WHERE """
		+ " AND ".join(conditions),
		params,
		as_dict=True,
	...
	best = None
	for r in rows:
		# The ORDER's length is the target. Pick rows keep the length the bucket
		# was graded at (allocation stamps the shelf's value), so they are only a
		# fallback for rows whose Sales Order Item carries no length.
		target = r.get("order_length") or r.get("pick_length")
		target_cm = _length_cm(target)
		if not target_cm:
			continue
		detail["candidates"].append({"opl": r.get("opl"), "target": target})
		if target_cm > graded_cm:
			continue  # upgrade -- this bunch is too short for that line
		if best is None or target_cm > best[1]:
			best = (target, target_cm, r.get("opl"))
		...
		if not grader or not packer:
			...
			if bunches_val <= 0:
				...
				frappe.response["message"] = {
					"status": "error",
					"message": "Enter a bunches count greater than zero.",
				}
			else:
	...
	try:
		...
		if not data:
			...
			if not box_label_name:
				...
				frappe.response["message"] = {
					"status": "error",
					"message": "Box label not found: " + box_label_name,
				}
			else:
				# Fetch box label
				...
				if box_doc.loaded == 1:
					...
					if already_added:
						frappe.response["message"] = {
							"status": "error",
							"message": "Box already on loading sheet",
						}
					else:
		...
		if not frappe.db.exists("Loading Sheet", ls_name):
			...
			for customer, box_so_pairs in by_customer.items():
				...
				for (so_name, v), d in var.items():
					...
					# spec_autofill.build_spec_rows / stock_movement.py).
					...
					boxes_ct = d["boxes"]
					farm_code = (
						frappe.db.get_value("Farm", so.get("farm"), "farm_code") if so.get("farm") else ""
					)
				...
				results.append(
					{
						...
						"action": action,
						"customer": customer,
						"sales_orders": sorted(orders_seen),
						"boxes": total_boxes,
					}
				)
...
def createOrUpdateFarmPackList():
	try:
		...
		for idx, entry in enumerate(items):
			...
			# bunch_id is NOT required for standard roses (manual entry). The
			# length is only required up front for those -- a scanned bunch gets
			# its length from its grading entry / pick row below.
			if not item_code or not bunch_uom:
				continue
			if not stem_length and not bunch_id:
				continue
			...
			if bunch_id:
				...
				# Pack the bunch at what the ORDER asked for, not at what its QR
				# label says, whenever this pick list took the bunch's own bucket
				# down to a shorter length. The label never changes, so refusing
				# the scan over that difference would make every downgraded
				# bucket unpackable. An upgrade is still refused -- there is no
				# way to pack a 50cm bunch into a 60cm line.
				target_length, target_detail = _bunch_packing_target(
					grading_stock_entry[0], opl_name=order_pick_list_id
				)
				if target_length:
					stem_length = target_length
				elif target_detail.get("upgrade_only"):
					wanted = ", ".join(
						sorted({c["target"] for c in target_detail["candidates"] if c.get("target")})
					)
					frappe.throw(
						_(
							"Stem length mismatch: bunch {0} is graded {1} but {2} needs {3}. "
							"A bunch can be packed into a shorter length, never a longer one."
						).format(
							bunch_id,
							target_detail.get("graded_stem_length") or "?",
							order_pick_list_id,
							wanted or "?",
						)
					)
			...
			# Resolve the pick row this bunch came from. Exact length first; a
			# downgraded bunch now carries the ORDER's length while its pick row
			# still carries the bucket's graded length, so fall back to variety +
			# bunch UOM before the whole-OPL default -- that default is row 1's
			# warehouse, which is simply wrong on a pick list spanning farms.
			source_warehouse = None
			for location in order_pick_list.table_ytkc:
				if (
					location.item_code == item_code
					and location.get("stem_length") == stem_length
					and location.uom == bunch_uom
				):
			...
			if not source_warehouse:
				for location in order_pick_list.table_ytkc:
					if location.item_code == item_code and location.uom == bunch_uom:
						source_warehouse = location.source_warehouse
						break
			...
			for key_tuple, vals in grouped_incoming.items():
				...
				for row in doc.pack_list_item:
					if (
						...
						and row.stem_length == stem_length
						and row.bunch_uom == bunch_uom
						and str(row.box_id or "") == str(box_id)
					):
			...
			for item in processed_items:
				if item.get("bunch_id"):
					stock_entries = frappe.get_all(
						"Stock Entry", filters={"custom_bunch_id": item["bunch_id"]}, limit=1
					)
					if stock_entries:
...
def setPackListBoxUnderPackReason():
	...
	try:
		...
		fpl_name = frappe.db.get_value("Farm Pack List", {"order_pick_list": order_pick_list_id})
		if not fpl_name:
...
def createStagingEntry():
	...
	if "status" not in frappe.response or frappe.response["status"] not in ["success", "error", "duplicate"]:
		try:
			...
			pass
		frappe.response.update(
			{"status": "error", "message": "Unexpected server error while processing box scan"}
		)
			...
			for it in ls_doc.items:
				...
				if key not in orders_by_so:
					...
					if so_name and frappe.db.exists("Sales Order", so_name):
						order_name = (
							frappe.db.get_value("Sales Order", so_name, "custom_order_name") or so_name
						)
					orders_by_so[key] = {
						"sales_order": so_name,
...
def fetchLoadingData():
	...
	try:
		...
		available_plans = [{"name": ap.name, "vehicle": ap.vehicle or ""} for ap in all_plans]
		...
		def get_opl_stats(so, opl):
			...
			if guide_rows:
				...
				packrate_data = frappe.get_all(
					"Pick List Item", filters={"parent": opl_name}, fields=["packrate"], limit_page_length=1
				)
				packrate = 200
				if packrate_data and packrate_data[0].packrate:
					packrate = int(packrate_data[0].packrate or 200)
				boxes_allocated = -(-total_stems // packrate) if packrate > 0 and total_stems > 0 else 0
			...
			if pli_row:
				...
				if soi_name:
					soi_flags = frappe.db.get_value(
						"Sales Order Item", soi_name, ["custom_mixed_box", "custom_mixed_bunch"], as_dict=True
					)
					if soi_flags:
		...
		if loading_plan:
			sorted_items = sorted(loading_plan.loading_plan_items, key=lambda x: x.loading_position or 0)
			...
			for (customer, delivery_point), stop_rows in stops.items():
				so_filters = {"delivery_date": tomorrow, "docstatus": 1, "customer": customer}
...
def fetchStockEntryByBunch():
	try:
		...
		# The pick list being packed, when the app knows it. Optional: without it
		# the bunch's bucket is matched against every submitted pick list holding
		# it, which is the same answer for all but the rare re-allocated bucket.
		opl_name = (
			data.get("custom_order_pick_list")
			or data.get("order_pick_list")
			or data.get("opl_name")
			or data.get("pick_list_id")
		)
		...
		if not stock_entries:
			...
			items = frappe.get_all(
				"Stock Entry Detail",
				filters={"parent": stock_entry.name, "parenttype": "Stock Entry"},
			...
			# A downgraded bucket is packed at the ORDER's length while every bunch
			# in it still wears the QR label it was graded with. Report the length
			# this bunch may actually be packed at, so scanning the unchanged
			# label does not read as a mismatch; graded_stem_length keeps the
			# label's own value for display. An upgrade resolves to nothing and
			# still shows the graded length, so it still fails the app's check.
			packing_length, packing_detail = _bunch_packing_target(stock_entry, opl_name=opl_name)
			...
			result = {
				...
				"stem_length": packing_length
				or stock_entry.custom_stem_length,  # Note: matches fromJson expectation
				# What the bunch was graded at, i.e. what its QR label reads.
				"graded_stem_length": stock_entry.custom_stem_length,
				"bucket": stock_entry.custom_bucket_id,
				"downgraded": bool(packing_detail.get("is_downgrade")),
		...
		frappe.response["message"] = {"error": True, "message": str(e)}
	...
	try:
		...
		if not matching_opls:
			...
			for opl in frappe.get_all(
				"Order Pick List", filters={"name": ["in", matching_opls]}, fields=["name", "team"]
			):
...
def getSchedulerData():
	...
	try:
		...
		for opl in opls:
			...
			while k < len(locs):
				...
				if si and si in soi_map and si not in seen_si:
					...
					specs = specs + [
						{
							"spec": sr.get("custom_line"),
							"variety": sr.get("item_code"),
							"length": sr.get("custom_length"),
							"box_type": sr.get("custom_box_type"),
							"boxes": bx,
						}
					]
				k = k + 1
		...
		frappe.response["message"] = {"success": True, "data": opls, "date": delivery_date}
	except Exception as e:
...
def getTransferScheduleData():
	...
	for r in rows:
		...
		if opl not in orders:
			...
			orders[opl] = {
				...
				"schedule": sm.get("seq") or 0,
				"team": sm.get("team") or (r.get("team") or ""),
				"total_buckets": 0,
				"total_stems": 0,
			}
			farm_agg[opl] = {}
	...
	for v in veh:
		...
		vehicles.append(
			{"name": v["name"], "trolleys": t, "buckets_per_trolley": b, "capacity_buckets": t * b}
		)
		...
		trips.append(
			{
				...
				"trip_date": str(t.get("trip_date") or ""),
				"status": t.get("status") or "Draft",
				"notes": t.get("notes") or "",
				"collection_order": t.get("collection_order") or "",
				"farm": t.get("farm") or "",
				"total_buckets": int(t.get("total_buckets") or 0),
				"total_stems": int(t.get("total_stems") or 0),
				"capacity_buckets": int(t.get("capacity_buckets") or 0),
				"orders": items,
				"dispatched_at": str(t.get("dispatched_at") or ""),
				"received_at": str(t.get("received_at") or ""),
			}
		)
	...
	dist_rows = frappe.get_all(
		"Farm Distance",
		fields=["name", "from_farm", "to_farm", "distance_km", "is_road_leg"],
		limit_page_length=0,
	)
	distances = []
	for d in dist_rows:
		distances.append(
			{
				"name": d.get("name") or "",
				"a": d.get("from_farm") or "",
				"b": d.get("to_farm") or "",
				"km": float(d.get("distance_km") or 0),
				"leg": 1 if int(d.get("is_road_leg") or 0) else 0,
			}
		)
	...
	try:
		...
		for location in order_pick_list.get("table_ytkc", []):
			...
			if key in consolidated_locations:
				consolidated_locations[key]["qty"] = consolidated_locations[key]["qty"] + location.get(
					"qty", 0
				)
				consolidated_locations[key]["stock_qty"] = consolidated_locations[key][
					"stock_qty"
				] + location.get("stock_qty", 0)
				consolidated_locations[key]["picked_qty"] = consolidated_locations[key][
					"picked_qty"
				] + location.get("picked_qty", 0)
				consolidated_locations[key]["stock_reserved_qty"] = consolidated_locations[key][
					"stock_reserved_qty"
				] + location.get("stock_reserved_qty", 0)

				if "custom_buckets" not in consolidated_locations[key]:
					consolidated_locations[key]["custom_buckets"] = [
						consolidated_locations[key].get("bucket")
					]
		...
		for idx, location in enumerate(order_pick_list["table_ytkc"], start=1):
			...
			if so_item_ref:
				spec_row = frappe.db.get_value(
					"Sales Order Item", so_item_ref, ["custom_line", "custom_length"]
				)
				if spec_row:
		...
		if variety_codes:
			for it in frappe.get_all(
				"Item", filters={"name": ["in", variety_codes]}, fields=["name", "item_group"]
			):
		...
		farm_pack_list_names = frappe.db.get_all(
			"Farm Pack List",
			filters={"order_pick_list": pick_list_id},
			fields=["name"],
			order_by="creation desc",
			limit=1,
		)
		...
		if sales_order:
			...
			stems_per_bunch_by_item = {it.item_code: _uom_factor(it.uom) for it in so.items if it.item_code}
			...
			if guide_rows:
				...
				if not is_mixed:
					for bunch_item in so.items:
						...
						if bunch_cl and frappe.db.exists(
							"Spec Box Item", {"parent": bunch_cl, "bunch_type": "Mixed Bunch"}
						):
							is_mixed_bunch = True
							break
			...
			if guide_rows:
				...
				for item in relevant_items:
					...
					if ordered_stems is not None:
						...
						stems = qty * conv
					planned_stems_by_item[item.item_code] = (
						planned_stems_by_item.get(item.item_code, 0) + stems
					)
					planned_total += stems
				...
				calculated_boxes = (
					round(planned_total / packrate_mixed_box_sum, 1) if packrate_mixed_box_sum > 0 else 0
				)
				planned_boxes = planned_boxes_from_order or calculated_boxes
				packrate_per_box = (
					round(planned_total / planned_boxes)
					if planned_boxes > 0
					else safe_int(packrate_mixed_box_sum)
				)
			...
			# =========================================================
				...
				calculated_boxes = (
					round(planned_total / packrate_per_box, 1)
					if packrate_per_box > 0 and planned_total > 0
					else 0
				)
				planned_boxes = planned_boxes_from_order or calculated_boxes
			...
			# =========================================================
				...
				packrate_per_box = (
					round(planned_total / planned_boxes) if planned_boxes > 0 else safe_int(planned_total)
				)
			...
			try:
				relevant_items = [
					item for item in so.items if not opl_so_item_names or item.name in opl_so_item_names
				]
				...
				if is_mixed_bunch_spec:
					recipe_items = [
						item for item in relevant_items if item.get("custom_mixed_bunch") == 1
					] or relevant_items
						...
						bunch_guide.append(
							{
								...
								"variety_name": (meta and meta.get("item_name"))
								or item.item_name
								or item.item_code,
		...
		if schedule_blocked:
			...
			if not stock_entries:
				...
				if current_issued_to and current_issued_to != sale_order_item and not allocated_to_this_item:
					...
					# -------------------------------
					frappe.db.set_value("Stock Entry", stock_entry_name, "custom_issued_to", sale_order_item)
					...
					for item in pick_list_items:
						...
						frappe.db.set_value("Pick List Item", item.name, "issued", 1)
					...
					for item in shelf_items:
						...
						try:
							existing_log = frappe.db.get_value(
								"Shelving Log", {"shelf_item": item.name}, "name"
							)
							if existing_log:
								frappe.db.set_value(
									"Shelving Log",
					...
					farm = next((p.farm for p in pick_list_items if p.farm), None) or frappe.db.get_value(
						"Stock Entry", stock_entry_name, "farm"
					)
...
def shelveBucket():
	...
	if not shelf_id:
		_fail("shelf_id_not_null", "Shelf ID is missing.")
		return
	if not bucket_id:
		_fail("bucket_id_not_null", "Bucket ID is missing.")
		return
	if not farm:
		_fail("farm_not_null", "Farm is missing.")
		return
	...
	for item in shelf_doc.items or []:
		if (item.bucket_id or "").lower() == bucket_id.lower():
			_fail("duplicate_entry", "The bucket has already been shelved.")
			return

	# duplicate on ANOTHER shelf (transfer buckets self-heal, otherwise block)
	other_shelves = frappe.get_all(
		"Shelf Item", filters={"bucket_id": bucket_id, "parent": ["!=", shelf_id]}, fields=["name", "parent"]
	)
	if other_shelves:
		...
		if transfer_rows:
			...
			_fail(
				"duplicate_entry",
				"The bucket has already been shelved on shelf {0}.".format(other_shelves[0].parent),
			)
			return
	...
	if len(existing_buckets) >= 2:
		_fail("two_buckets_per_shelf", "The shelf is full.")
		return
	...
	if not harvest_entry:
		frappe.response["data"] = {
			...
			"message": "No harvesting entry found for bucket {0} within 1 day of receiving date ({1}).".format(
				bucket_id, recv_date
			),
	...
	if gap > 1:
		frappe.response["data"] = {
			...
			"message": "Bucket harvested on {0} but received on {1} ({2} days apart). Maximum allowed gap is 1 day.".format(
				harvest_date, recv_date, gap
			),
			"payload": {
				"bucket_id": bucket_id,
	...
	if days_since > max_allowed_days:
		frappe.response["data"] = {
			...
			"message": "Cannot shelf bucket — received on {0} ({1} days ago). Maximum allowed for {2} is {3} day(s).".format(
				recv_date, days_since, origin_farm, max_allowed_days
			),
			"payload": {
				"bucket_id": bucket_id,
				"origin_farm": origin_farm,
				"received_on": str(recv_date),
				"days_since_receiving": days_since,
...
def _shelve_update_bas(bucket_id, variety, farm, shelf_id, result):
	...
	try:
		bas_name = frappe.db.get_value(
			"Bucket Allocation Status", {"bucket_id": bucket_id, "item_code": variety}, "name"
		)
		if bas_name:
			bas = frappe.get_doc("Bucket Allocation Status", bas_name)
			if bas.in_transit == 1:
...
def _shelve_check_submit_opl(bucket_id, result):
	...
	try:
		for row in frappe.db.sql(
			"SELECT DISTINCT parent FROM `tabPick List Item` WHERE bucket = %s", bucket_id, as_dict=True
		):
...
def createBucketDispatch():
	...
	try:
		...
		if not frappe.db.exists("Farm", target_farm):
			...
			frappe.response["message"] = {
				"status": "error",
				"message": _("Unknown farm: {0}").format(target_farm),
			}
			return
		if not frappe.db.exists("Vehicle", vehicle):
			...
			frappe.response["message"] = {
				"status": "error",
				"message": _("Unknown vehicle: {0}").format(vehicle),
			}
			return
		...
		warning = ""
		if is_internal:
			warning = _("{0} is flagged as a customer-delivery truck, not internal logistics.").format(
				vehicle
			)
			...
			dispatches.append(
				{
					...
					"buckets": [
						{"bucket_id": b.bucket_id, "received": int(b.received or 0)} for b in doc.buckets
					],
				}
			)
...
def receiveBucketDispatch():
	...
	try:
		...
		open_names = frappe.get_all(
			"Bucket Dispatch", filters=filters, fields=["name"], order_by="dispatch_datetime asc"
		)
