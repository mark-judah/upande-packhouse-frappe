...
window.PriceListTree = (function () {
	...
	function render(container, state, onSetPrice, saveBarMount) {
		...
		html +=
			...
			'<span class="plt-summary-n">' +
			(state.priced_cells || 0) +
			"</span> / " +
			'<span class="plt-summary-n">' +
			(state.total_cells || 0) +
			"</span> variety&times;length combinations priced" +
		...
		html +=
			'<div class="plt-head-row">' +
			'<div class="plt-head-name">Variety</div>' +
			lengths
				.map(function (l) {
					return '<div class="plt-head-len">' + esc(l) + "</div>";
				})
				.join("") +
			"</div>";
			...
			line.cats.forEach(function (cat) {
				...
				cat.items.forEach(function (it) {
					...
					lengths.forEach(function (len) {
						...
						html +=
							'<div class="plt-cell' +
							(missing ? " plt-missing" : " plt-priced") +
							'" data-code="' +
							esc(it.code) +
							'" data-length="' +
							esc(len) +
							'">' +
							(missing ? "&mdash;" : fmtRate(rate, currency)) +
							"</div>";
					});
					html += "</div>";
				});
	...
	function nextEditableCell(container, code, length, lengths) {
		...
		return container.querySelector(
			'.plt-cell[data-code="' + cssEsc(code) + '"][data-length="' + cssEsc(nextLen) + '"]'
		);
	}
	function cssEsc(s) {
	...
	function wireCells(container, barHost, onSetPrice, currency, pending) {
		var lengths = Array.prototype.slice
			.call(container.querySelectorAll(".plt-head-len"))
			.map(function (h) {
		...
		function openEditor(cell, focusAfterOpen) {
			...
			var current = staged
				? staged.rawValue
				: cell.classList.contains("plt-missing")
				? ""
			...
			var stageAndClose = function (moveNext) {
				...
				if (unchanged) {
					...
					renderCellIdle(
						cell,
						cell.classList.contains("plt-missing") ? null : Number(original),
						currency,
						false
					);
				} else {
			...
			input.addEventListener("keydown", function (e) {
				...
				if (e.key === "Escape") {
					...
					if (staged) renderCellIdle(cell, staged.rawValue, currency, true, true);
						...
						renderCellIdle(
							cell,
							cell.classList.contains("plt-missing") ? null : Number(current),
							currency,
							false
						);
				}
			...
			if (btn.getAttribute("data-act") === "plt-discard") {
				Object.keys(pending).forEach(function (key) {
					...
					if (cell)
						renderCellIdle(
							cell,
							p.original ? Number(p.original) : null,
							currency,
							false
						);
				});
	...
	function renderCellIdle(cell, value, currency, dirty, isRaw) {
		...
		cell.className =
			"plt-cell" + (missing ? " plt-missing" : " plt-priced") + (dirty ? " plt-dirty" : "");
	...
	function saveAllPending(container, barHost, onSetPrice, currency, pending) {
		...
		Promise.all(
			...
			results.forEach(function (r) {
				...
				if (r.res && r.res.success) {
					...
					if (r.res.cleared) renderCellIdle(r.cell, null, currency, false);
						...
						renderCellIdle(
							r.cell,
							r.res.rate != null ? r.res.rate : Number(r.p.rawValue),
							currency,
							false
						);
				} else {
					...
					setTimeout(function () {
						r.cell.classList.remove("plt-error");
					}, 1800);
			...
			if (bar) {
				var label = bar.querySelector(".plt-savebar-n");
				if (label && failed)
					label.textContent += " \u2014 " + failed + " failed, click Save to retry";
			}
		});
	}
