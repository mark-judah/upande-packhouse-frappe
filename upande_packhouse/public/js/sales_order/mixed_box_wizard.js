...
function extract_mixes_from_items(frm) {
	...
	let group_numbers = Object.keys(group_map)
		.map(Number)
		.sort((a, b) => a - b);
...
function open_multi_mix_wizard(frm, edit_mode) {
	...
	function validate_and_submit() {
		...
		if (duplicate) {
			frappe.throw(
				__(
					'Mix Name "{0}" is used by more than one group. Each group must have a unique name.',
					[duplicate[0]]
				)
			);
			return;
		...
		for (let i = 0; i < mixes.length; i++) {
			...
			if (total_per_box !== packrate_val) {
				frappe.throw(
					__("Group {0}: Packrate mismatch – expected {1} stems, got {2}", [
						i + 1,
						packrate_val,
						total_per_box,
					])
			...
			for (let v of values.varieties) {
				...
				if (!v.custom_length) {
					frappe.throw(
						__("Group {0}: Length is required for {1}", [i + 1, v.item_code])
					);
					return;
				}
				if (!v.stems_per_box || cint(v.stems_per_box) < 1) {
					frappe.throw(
						__("Group {0}: Stems / Box is required for {1}", [i + 1, v.item_code])
					);
					return;
				}
				...
				if (nob < 1 || nob > cint(values.number_of_boxes)) {
					frappe.throw(
						__("Group {0}: Invalid # boxes for {1} – must be between 1 and {2}", [
							i + 1,
							v.item_code,
	...
	function load_group(idx) {
		...
		let title_suffix =
			data.mix_name && String(data.mix_name).trim()
				? `– ${data.mix_name}`
				: `– Group ${idx + 1}`;
		...
		if (edit_mode) {
			...
			dialog.$wrapper
				.find("#btn-next-group")
				.text(idx === mixes.length - 1 ? "Add New Group →" : "Next Group →");
		}
