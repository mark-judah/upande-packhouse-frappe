...
function open_search_picker(title, options, onPick, hint) {
	...
	d.fields_dict.body.$wrapper.html(`
        <div class="sod">
            ...
            <div class="sod-search"><input type="text" placeholder="${__(
				"Search…"
...
async function open_consignee_picker(frm) {
	...
	open_search_picker(__("Select Consignee"), options, (selected) => {
		...
		frappe.show_alert(
			{ message: __("Consignee set to {0}", [selected]), indicator: "green" },
			3
		);
	});
}
...
async function open_shipping_agent_picker(frm) {
	...
	try {
		...
		/* delivery point missing — fall through to every agent */
	}
	...
	open_search_picker(
		...
		(selected) => {
			...
			frappe.show_alert(
				{ message: __("Shipping agent set to {0}", [selected]), indicator: "green" },
				3
			);
		},
		scoped
			? null
			: __("No agents curated for this Delivery Point yet — showing every Shipping Agent")
	);
}
